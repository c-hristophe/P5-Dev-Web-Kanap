...
document.getElementsByClassName("cart__item__content__settings__quantity")
imput.classList.add('itemQuantity');
...
[ p3, imput ].forEach(child => div4.appendChild(child));
...
[imput].forEach(child => imput.setAttribute("name", "itemQuantity"));
[imput].forEach(child => imput.setAttribute("min", "1"));
[imput].forEach(child => imput.setAttribute("max", "100"));
[imput].forEach(child => imput.setAttribute("value", data[pas].qte));
...
const imputParent = document.querySelector("cart__item__content__description"); 
    imput.innerText = qte;
...
let quantite = imput.getAttribute("value");
